...
build:
	docker buildx build --build-arg VERSION=$(VERSION) --build-arg ARCH=$(ARCH) --platform $(PLATFORM) -t $(ASSET_NAME) -f $(ASSET_WORKING_DIRECTORY)/Dockerfile $(BUILD_WORKING_DIRECTORY)
push:
	docker buildx tag $(ASSET_NAME):latest $(ASSET_TARGET_URL)
	docker buildx push $(ASSET_TARGET_URL)